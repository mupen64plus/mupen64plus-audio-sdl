                                            ...
                                            const char* resampler_id)
{
    ...
    sdl_backend->frequency = default_frequency;
...
void release_sdl_backend(struct sdl_backend* sdl_backend)
{
    ...
    if (sdl_backend->primary_buffer != NULL) {
        free(sdl_backend->primary_buffer);
    }

    /* release mix buffer */
    if (sdl_backend->resample_buffer != NULL) {
...
void sdl_apply_volume(struct sdl_backend* sdl_backend, int volume)
{
    ...
    {
        SDL_SetAudioStreamGain(sdl_backend->stream, volume / 100.0f);
    }
}
